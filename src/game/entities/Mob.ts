...
export class Mob extends Phaser.Physics.Arcade.Sprite implements AiActor, Damageable {
  ...
  tick(time: number, delta = 16): void {
    ...
    // Under the ground it stands on, which after their change is `y + footLift`
    // rather than `y` — a shadow at the sprite centre would sit inside the mob.
    if (this.visible) this.shadow.sync(this.x, this.y + this.footLift);
